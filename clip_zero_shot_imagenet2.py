...
imagenet_train_dataloader = create_dataloader(IMAGENET_TRAIN_FOLDER)
imagenet_val_dataloader = create_dataloader(IMAGENET_VAL_FOLDER)
preds_10, targets_10 = get_preds_targets(
    model_10, classifier_10, imagenet_val_dataloader, args, topk=topk)

# %%

tot_preds_10 = [preds_10[i] for i in range(len(preds_10))]
tot_preds_10 = torch.cat(tot_preds_10, dim=1)
tot_targets_10 = [targets_10[i].reshape(1,1) for i in range(len(targets_10))]
tot_targets_10 = torch.cat(tot_targets_10, dim=1)
tot_correct_10 = tot_preds_10.eq(tot_targets_10.expand_as(tot_preds_10))
print("For model 10:")
for i in topk:
...
preds_11, targets_11 = get_preds_targets(
    model_11, classifier_11, imagenet_val_dataloader, args, topk=topk)
...
correct_10 = tot_correct_10[0].cpu().numpy()
target_10 = tot_targets_10[0].cpu().numpy()
correct_11 = tot_correct_11[0].cpu().numpy()
target_11 = tot_targets_11[0].cpu().numpy()
for i in range(IMAGENET1K_COUNT):
    class_correct_10[i] = np.sum(correct_10[target_10==i])
    class_total_10[i] = np.sum(target_10==i)

    class_correct_11[i] = np.sum(correct_11[target_11==i])
    class_total_11[i] = np.sum(target_11==i)
# %%
...
for i in range(n):
    print(f"{imagenet_classes_short[ind[i]]}: \
\n11: {class_percent_11[ind[i]]:.4f}, 10: {class_percent_10[ind[i]]:.4f}, diff: {class_percent_diff[ind[i]]:.4f}")

# %%
k = np.load(IMAGENET_PREDS_RUN10)
# %%
k.shape
# %%
k
